package com.vendo.auth_service.adapter.verification.in;

import com.vendo.auth_service.adapter.verification.out.mapper.ValidateMapper;
import com.vendo.auth_service.application.auth.EmailVerificationService;
import com.vendo.auth_service.adapter.verification.in.dto.ValidateRequest;
import jakarta.validation.Valid;
import lombok.RequiredArgsConstructor;
import org.springframework.web.bind.annotation.*;

@RestController
@RequiredArgsConstructor
@RequestMapping("/verification")
public class VerificationController {

    private final ValidateMapper validateMapper;

    private final EmailVerificationService emailVerificationService;

    @PostMapping("/send-otp")
    void sendOtp(@RequestParam String email) {
        emailVerificationService.sendOtp(email);
    }

    @PostMapping("/resend-otp")
    void resendOtp(@RequestParam String email) {
        emailVerificationService.resendOtp(email);
    }

    @PostMapping("/validate")
    void validate(
            @RequestParam String otp,
            @Valid @RequestBody ValidateRequest validateRequest
    ) {
        emailVerificationService.validate(otp, validateMapper.toCommand(validateRequest));
    }

}

